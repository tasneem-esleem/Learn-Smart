...
import { useNavigate } from "react-router-dom";
 export const GradeHeader = ({ title }) => (
  <div className="flex items-center my-10 px-28">
    <div className="flex-grow h-[1px] bg-gray-300"></div>

    <span className="px-4 text-gray-400 text-sm font-medium tracking-wider">
      {title}
    </span>
...
export default function Books({ moreBooks = false , data = null ,showGradeHeader = false }) {
  ...
  const finalBooks = data ? data : (moreBooks ? books : books.slice(0, 8));
  return (
    <section className="bg-white py-20 px-4 mb-10">
      <div className="max-w-7xl mx-auto w-full">
        {!data && (
           <div className="text-center mb-12">
             <h2 className="text-3xl md:text-4xl font-bold text-gray-900">Explore Books for Every Subject</h2>
           </div>
        )}
        {showGradeHeader && (
          <div className="-mt-20 ">
            ...
            >
              <div className="w-full h-[380px] overflow-hidden">
                <img
                  src={book.cover}
                  alt={book.title}
                  className="w-[320px] h-full object-cover block"
                  style={{
                    objectFit: "cover",
              ...
              <div className="p-6 text-center flex flex-col flex-grow">
                <h3 className="text-[17px] font-bold text-gray-800 mb-1 leading-tight">
                  {book.title}
                </h3>
                ...
                <div className="mt-auto">
                  <button onClick={() => navigate(`/book/${book.id}`)} className="bg-[#42b893] hover:bg-[#369c7c] text-white font-medium py-2.5 px-10 rounded-full transition-colors duration-300 text-[14px]">
                    View details
                  </button>
                </div>
              </div>
       ...
       {!moreBooks && !data && (
          <div className="flex justify-center mt-12">
             <button onClick={() => navigate("/all-books")} className="bg-[#42b893] hover:bg-[#369c7c] text-white font-bold py-3 px-14 rounded-full transition-all duration-300 ease-in-out shadow-[0_4px_14px_0_rgba(66,184,147,0.39)] hover:shadow-[0_6px_20px_rgba(66,184,147,0.23)] hover:scale-105 active:scale-95">More Book</button>
          </div>
        )}
      </div>
    </section>

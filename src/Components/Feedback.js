...
export default function Feedback() {
  ...
  const handleSend = async () => {
    if (!enjoy) {
      setServerError('Please select whether you are enjoying the platform or not.');
      setStatus('error');
      return;
    }
    ...
    try {
      ...
      const res = await fetch('https://api-zyzn.onrender.com/api/feedback', {
        method: 'POST',
        headers: {
    ...
    <div className="w-full min-h-screen bg-white p-4 sm:p-8 md:p-12 lg:p-16 py-12 md:py-16 lg:py-20 font-sans flex justify-center">
      <div className="bg-white w-full max-w-7xl rounded-[40px] shadow-[0_10px_60px_rgba(0,0,0,0.05)] p-6 sm:p-10 md:p-14 h-fit border border-gray-50">
        ...
        <div className="flex items-center gap-3 mb-10">
          ...
          <h1 className="text-lg md:text-xl font-bold text-black">Send Feedback</h1>
        </div>

        {/* Title */}
        <div className="text-center mb-10">
          <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-1">Give Feedback</h2>
          <p className="text-xs text-gray-400">How to satisfy you with your experience with us</p>
        </div>
        ...
        <div className="mb-10">
          ...
          <div className="flex items-center gap-8">
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className={`text-sm font-medium transition-colors ${enjoy === "yes" ? "text-gray-900" : "text-gray-400 group-hover:text-gray-600"}`}>Yes</span>
            </label>
            ...
            <label className="flex items-center gap-3 cursor-pointer group">
              ...
              <span className={`text-sm font-medium transition-colors ${enjoy === "no" ? "text-gray-900" : "text-gray-400 group-hover:text-gray-600"}`}>No</span>
            </label>

...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      const res = await fetch(
        "https://api-zyzn.onrender.com/api/notifications",
        {
  ...
  if (loading)
    return (
      <div
        className="bg-white min-h-screen py-6 px-4 md:py-10 md:px-28 animate-pulse"
        dir="ltr"
      >
        <div className="max-w-7xl mx-auto space-y-4">
          ...
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-24 w-full bg-gray-50 rounded-[24px]"
            ></div>
          ))}
        </div>
      </div>
  ...
  return (
    <div
      className="bg-white min-h-screen py-6 px-4 md:py-10 md:px-12 lg:px-28"
      dir="ltr"
    >
            ...
            notifications.map((n) => (
              <div
                ...
                className={`flex items-start sm:items-center justify-between p-4 md:p-6 rounded-[20px] md:rounded-[28px] transition-all border  shadow-lg
                  ${!n.read ? "bg-[#f8fdfb] shadow-lg border-[#eafaf4]  " : "bg-white border-gray-50 opacity-90 shadow-lg"}`}
              >
